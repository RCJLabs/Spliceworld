...
- **R21 — Splice-Dex completeness.** `combos.json`'s own `_doc` says discoveries are "permanently logged in the save for the Splice-Dex"; the dex has `parts`/`enemies`/`traits`/`variants` and no `combos`. Rivals have no page; the family tree is one generation deep. *Done when: everything the game announces is findable again afterwards.*
- **R22 — An enemy AI with a policy.** `enemyChooseMove` picks a random affordable move with a 75% bias toward damaging ones: no targeting, no class awareness, no finishing a hurt opponent. *Done when: the same roster plays measurably better and the all-grade balance gate still passes.* Largest blast radius in the queue — it moves every number in the balance table.
- **R23 — Active hides and organs.** 59 of 184 parts are passive stat sticks; hides carry a move 0/32 times and organs 5/32, so two of six sockets contribute nothing to play. Hides want defensive reactions, organs want utility actives — which is what R20's orphaned keywords were written for. *Done when: a hide and an organ each change how a fight is played.*
- **R24 — Mutation traits.** The allele machinery is generic and exactly one trait uses it (`dense_bones`). *Done when: two equally-starred parents can produce visibly different offspring, and the difference shows in a fight.*
- **R25 — Facility depth.** Two tracks exist (Theater ×2, Containment ×3); the Gene Scanner, Extractor efficiency, incubator slots and infirmary speed named in §3.10 do not. *Done when: money has a second sink that changes the loop, and each track pays back measurably.*
- **R26 — A second region + Threat Gen 3.** One region, five nodes, eight encounters is the whole campaign. *Done when: taking Greenfield opens a region whose fights need different anatomy than the one that won the first.* Directly answers §8 risk 5.
- **R27 — Rival geneticists as a ladder.** Three rivals, and they counter-bias through their own path rather than the director's banked usage data. *Done when: a rival you have beaten twice fields something built to answer your actual stable.*
- **R28 — Battle readability.** `turnForecast` returns speed and nothing else. After R18 the class triangle decides fights and the player gets no pre-commit signal about it. *Done when: a new player can predict super-effective before pressing, at 380px.*
- **R29 — Onboarding for the other eight systems.** Onboarding is five steps ending at first conquest (the M7 criterion); breeding, the chaos vat, rehabilitation, operations, contestation, scars, temperament and the jobs board all shipped since and are unguided. *Done when: every shipped system has a first-use guide derived from state, and none fires before its system is reachable.*
