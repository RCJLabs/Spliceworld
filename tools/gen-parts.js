...
//
// `data/notes/parts.md` says hand-authored content lives in this
...
// R81 — the geometry now lives in its own file, so the salvage parts have to
// be reassembled from both halves before they can be written back out.
const existingShapes = JSON.parse(readFileSync(join(root, 'data/parts-shapes.json'), 'utf8')).shapes;
const salvage = existing.parts
  .filter((p) => p.species === 'salvage')
  .map((p) => ({ ...p, shapes: p.shapes ?? existingShapes[p.id] ?? [] }));

const out = {
  // R130 — NO `_doc` HERE ANY MORE, and the trap it used to carry is worth
  // remembering: this key was built as `existing._doc.split(...)` + a fresh
  // paragraph, so every regeneration re-appended the same text and twenty-one
  // copies had piled up before anybody looked. A generated file that reads
  // its own prose back in order to rewrite it is a loop with no fixed point.
  // The note lives in `data/notes/parts.md` now, which nothing generates.
  parts: [...parts, ...salvage].map(({ shapes, ...rest }) => rest),
};
// R81 — TWO FILES, because 69% of this one was geometry nobody needs to see
// the game start. `parts.json` is what every system reads to decide what a
// part IS; `parts-shapes.json` is what the renderer reads to draw it, and
// nothing else in the codebase touches it. The pair is written together and
// smoke asserts they stay a pair, so a part can never have stats with no
...
// what the note has claimed since R20 and what stopped being true immediately
// afterwards.
