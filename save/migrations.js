// R101 — FORTY-FOUR STEPS NOBODY ON THE CURRENT VERSION EVER RUNS.
//
// These are the migrations, lifted out of `save/save.js` unchanged. They
// were half that file — 26 KB of the 47 KB every player downloaded before
// the ranch could paint — and a save already on the current version runs
// none of them. `migrate` now imports this module only when it finds a save
// that is actually behind, so the common load path never fetches it at all.
//
// SELF-CONTAINED ON PURPOSE, and it always was: not one of these functions
// reads anything from module scope. A migration reproduces the schema of
// its own era, so it must not read today's TUNING, today's content, or
// today's `newGameState` — the historical shape is the whole point, and a
// migration that drifts with the tuning table rewrites the past. That is
// why this file has no imports, and it should keep having none.
//
// THE ASCENT RULE IS UNTOUCHED. Splitting the table changed no schema, no
// save version and no behaviour: `tools/saves.js` migrates a real save of
// every version v1–v45 — taken from the commit where that version was
// current — and lands each one on the shape a new game has.
